...
static const char ORIEN_URI[] PROGMEM = "http://www.opengis.net/def/property/OGC/0/PlatformOrientation";
...
void setup() {
  ...
  String bnoID = wifiID + ":BNO"  ;
  char bnoUID[bnoID.length()];
  for (i = 0; i < sizeof(bnoUID); i++) { 
      bnoUID[i] = bnoID[i]; 
    } 
  //------------------------------------------ //

  // set s2 sensor metadata
  s1.setUniqueID(bnoUID);
  s1.setName("BNO055 - IMU - Float 2");
  s1.setDataRecordDef(ORIEN_URI);
  s1.setLabel("Float Orientation");
  s1.addTimeStampUTC();
  ...
  s1.addOrientationEuler(ORIEN_URI, "Float Orientation");
  ...
  sos1->registerDevice(&s1); // BNO055

  Serial.println("BNO055 Example");
  delayTime = 60000;
  ...
  if (timeStatus() != timeNotSet) {
    if (now() != prevTime) { //update only if time has changed
     ...
     // Get orientation event and print its values.
      ...
      bno.getEvent(&event);
      sos1->pushFloat(event.orientation.z);
      sos1->pushFloat(event.orientation.y);
      sos1->pushFloat(event.orientation.x);
