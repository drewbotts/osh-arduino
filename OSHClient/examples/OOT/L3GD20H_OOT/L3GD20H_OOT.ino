...
void setup() {
  ...
  s1.setUniqueID(angVelUID);
  s1.setName("L3GD20H");
  s1.setDataRecordDef("urn:darpa:oot:message:angularvel");
  s1.setLabel("Angular Velocity");
  s1.addTimeStampUTC();
  ...
  Serial.println("L3GD20H Example");
...
void loop(void) {
  if (timeStatus() != timeNotSet) {
    if (now() != prevTime) { //update only if time has changed
      ...
      sos1->pushFloat(event.gyro.x);
      Serial.print("Y: ");
      Serial.print(event.gyro.y);
      Serial.println(" rad/s");
      sos1->pushFloat(event.gyro.y);
      Serial.print("Z: ");
      Serial.print(event.gyro.z);
      Serial.println(" rad/s");
      sos1->pushFloat(event.gyro.z);
