...
void setup(void) {
  ...
  // set sensor metadata
  s1.setUniqueID("urn:osh:esp8266:ds18b20");
  s1.setName("DS18B20");
  s1.addTimeStampUTC();
  s1.addMeasurement("temp", TEMP_URI, "Cel", "Water Temperature");
  ...
  Serial.println("DS18B20 Sensor Example");
  delayTime = 1000;
  sensors.begin();
}
...
void loop(void) {
  ...
  if (timeStatus() != timeNotSet) {
    if (now() != prevTime) { //update only if time has changed
      ...
      Serial.println(" C");
      sos->pushFloat(sensors.getTempCByIndex(0));
